...
Metoden som blir benyttet for enhetstestingen er Arrange-Act-Assert.
* Arrange: Det er klargjøring av det som trengs i enhetstesten
* Act: Det er selve testen man utfører
* Assert: Verfiserer selve resultatet, ved hjelp av `self.assert`
Dette gjøres i hver funksjon i testklassen, som har positive og negative tester av funksjonen som skal bli testet.

Funksjonene som testes er hentet fra:
...
**Positiv test**  
Verifiserer at predikerte verdier endres over tid når dataen viser en tydelig lineær vekst. Dette bekrefter da at funksjonen lager en lineær linje som gir mening.


## Resultat
Alle testene passerte på rundt 1.50 sekunder totalt.
Dette betyr at begge funksjonene oppfører seg slik det forventes, og den håndterer alle typer inputs på en korrekt måte.
